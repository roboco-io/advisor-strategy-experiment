...
| `opus-solo` | Opus | — |
| `deleg-opus` | Opus | Sonnet(위임+검증) |

`deleg-opus`는 유튜버 **개발동생**의 Advisor Strategy 변형이다: 메인 세션이 **Advisor(판단·검증)**
역할로 루프를 소유하고, 구현을 **Opus 워커 서브에이전트**에 `Agent`/`Task`로 위임한 뒤 diff·curl로
직접 검증한다. 다른 arm(강한 모델이 조언, 약한 워커)과 달리 **워커가 강한 모델(Opus)**이다.
워킹스타일 규율은 [`CLAUDE.md`](CLAUDE.md)(§모델 역할 분담)와
[`.claude/agents/worker.md`](.claude/agents/worker.md)(`model: opus` 고정)에 박제돼 있다.
