...
1. **강한 모델은 실행자보다 플래너다.** 모델 강도 서열은 Fable > Opus > Sonnet > Haiku.
   최강 Fable이 직접 다 하면 꼴찌(45.1%)지만, **계획만 하고 실행을 Haiku에 넘기면 2위(49.1%)**.
   Plan-then-Execute 플래너 3종도 최강 Fable 플래너가 1위: Fable→Haiku **49.1%** >
   Sonnet→Opus 47.3% > Opus→Sonnet 46.6%.

2. **단, 플래너 강도와 품질이 단조 비례하진 않는다.** 서열 2위 Opus 플래너(→Sonnet)가 서열
   3위 Sonnet 플래너(→Opus)에도 밀렸고, **유일하게 부팅 실패(2/5)**로 실질 품질 최저(28.0%).
   플래너-실행자 **궁합**(실행자가 그 계획을 작동 코드로 옮기는 역량)이 신뢰성을 좌우한다.
